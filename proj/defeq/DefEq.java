...
public final class DefEq {
    ...
    public static final boolean termdefeq(ASTNode l, Env<ASTType> sl, ASTNode r, Env<ASTType> sr, AlphaEnv alpha, Env<ASTType> phi) {
        ...
        if (l instanceof ASTLet ln && r instanceof ASTLet rn)
            return typedefeq(ln.getDeclType(), sl, rn.getDeclType(), sr, alpha, phi)
        ...
        if (l instanceof ASTFunc ln && r instanceof ASTFunc rn)
            return typedefeq(ln.getArgtype(), ln.getSig(), rn.getArgtype(), rn.getSig(), alpha, phi)
    ...
    public static final boolean typedefeq(ASTType l, Env<ASTType> sl, ASTType r, Env<ASTType> sr, Env<ASTType> phi) {
        return typedefeq(l, sl, r, sr, new AlphaEnv(), phi);
    }

    public static final boolean typedefeq(ASTType l, Env<ASTType> sl, ASTType r, Env<ASTType> sr, AlphaEnv alpha, Env<ASTType> phi) {
        ...
        if (l instanceof ASTTId lt && r instanceof ASTTId rt) {
            ...
            return typedefeq(phi.unfold(lt), sl, phi.unfold(rt), sr, alpha, phi);
        }
        
        if (l instanceof ASTTArrow lt && r instanceof ASTTArrow rt)
            return typedefeq(lt.getDom(), sl, rt.getDom(), sr, alpha, phi)
                && typedefeq(lt.getCodom(), sl, rt.getCodom(), sr, alpha.extend(lt.getId(), rt.getId()), phi);
        if (l instanceof ASTTLollipop lt && r instanceof ASTTLollipop rt)
            return typedefeq(lt.getDom(), sl, rt.getDom(), sr, alpha, phi)
                && typedefeq(lt.getCodom(), sl, rt.getCodom(), sr, alpha.extend(lt.getId(), rt.getId()), phi);
        
        if (l instanceof ASTTPair lt && r instanceof ASTTPair rt)
            return typedefeq(lt.getFirst(), sl, rt.getFirst(), sr, alpha, phi)
                && typedefeq(lt.getSecond(), sl, rt.getSecond(), sr, alpha.extend(lt.getId(), rt.getId()), phi);
        ...
        if (l instanceof ASTTUnion lt && r instanceof ASTTUnion rt) {
            ...
            for (String label : left.keySet()) {
                ...
                if (rightType == null || !typedefeq(leftType, sl, rightType, sr, alpha, phi)) return false;
            }
            return true;
        }
        ...
        if (l instanceof ASTTEq lt && r instanceof ASTTEq rt)
            return termdefeq(lt.getTerm1(), sl, rt.getTerm1(), sr, alpha, phi)
                ...
                && typedefeq(lt.getType(), sl, rt.getType(), sr, alpha, phi);
